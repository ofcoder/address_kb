...
class RestApi extends \IRestService
{
    public static function OnRestServiceBuildDescriptionHandler()
    {
        return [
            'kb' => [
                ...
                //v2 api
                'kb.v2.deactivate' => [
                    'callback' => [__CLASS__, 'deactivateUserV2'],
                    'options' => [],
                ],
                'kb.v2.crupd' => [
                    'callback' => [__CLASS__, 'crupdV2'],
                    'options' => [],
                ],
                'kb.v2.oschanges' => [
                    'callback' => [__CLASS__, 'departmentChangesV2'],
                    'options' => [],
                ],
                'kb.v2.oschangesclear' => [
                    'callback' => [__CLASS__, 'departmentChangesDelete'], // используем тот же метод что и на ручке kb.oschangesclear
                    'options' => [],
                ],
                //not changed api
                'kb.addtaskbyxml' => [
    ...
    public static function departmentChangesV2($query, $n, \CRestServer $server): array
    {
        $lastId = null;
        $departments = $users = [];
        $rs = DepartmentUpdateTable::getList();
        ...
        if (!$ar['UF_USER_ID']) {
            ...
            if ($ar['UF_USER_ID'] > 0) {
                $arUser = \Bitrix\Main\UserTable::getById($ar['UF_USER_ID'])->fetch();
                $ar['XML_ID'] = $arUser['XML_ID'];
            }
    ...
    public static function deactivateUserV2($query, $n, \CRestServer $server): array
    {
        $errors = RestApiHelpers::checkEmptyOrExist($query, ['XML_ID']);
        if ($errors) {
            CHTTP::SetStatus("404 Not Found");
            return [
                'error' => 'ERROR_CODE',
        ...
        $userId = RestApiHelpers::getUserByField('=XML_ID', $query['XML_ID']);
        $description = RestApiHelpers::deactivateById($userId);
    ...
    public static function crupdV2($query, $n, \CRestServer $server): array
    {
        $errors = RestApiHelpers::checkEmptyOrExist($query, ['XML_ID']);
        if ($errors) {
            CHTTP::SetStatus("404 Not Found");
        ...
        $findUser = RestApiHelpers::getUserByField('=XML_ID', $query['XML_ID']);
        $id = $findUser['ID'];
        ...
        if ($text['error']) {
            if (RestApiHelpers::userWithLoginOrEmailExists($query['LOGIN'], $query['EMAIL'])) {
                $text['error_description'] = '#USER_WITH_LOGIN_OR_EMAIL_EXISTS#' . $text['error_description'];
            }

            return [ $text['error_description'] ];
        }
...
------------------------------------------------------";
    ...
    */
    public static function addTaskByAsterisk($query, $n, \CRestServer $server)
    {
        ...
        \Bitrix\Main\Diag\Debug::dumpToFile($query, 'query', '/local/logs/1_logs.txt');
        \Bitrix\Main\Diag\Debug::dumpToFile($_SERVER['REQUEST_URI'], 'REQUEST_URI', '/local/logs/1_logs.txt');
        ...
        \Bitrix\Main\Diag\Debug::dumpToFile($smartIds, '$smartIds', '/local/logs/bp_logs.txt');
        $fields = [
        ...
        \Bitrix\Main\Diag\Debug::dumpToFile($fields, '$fields', '/local/logs/bp_logs.txt');
        $smartElemId = self::createSmartProcess($smartTypeId,$userId, $fields);
        \Bitrix\Main\Diag\Debug::dumpToFile($smartElemId, '$smartElemId', '/local/logs/bp_logs.txt');
        if($smartElemId)
        {

            \Bitrix\Main\Diag\Debug::dumpToFile($smartElemId, 'smartElemId', '/local/logs/1_logs.txt');
            return [
                'status' => 'OK',
    ...
    */
    private static function checkItemSmartProcess($smartTypeId = 158, $userId = 1, $filter=array())
    {
        ...
        //\Bitrix\Main\Diag\Debug::dumpToFile($elementsIds, '$elementsIds', '/local/logs/bp_logs.txt');
        return $elementsIds;
    }
